...
public:
  ...
  virtual void initialize_liquid() = 0;
    virtual void add_liquid(size_t x, size_t y) = 0;
    virtual void timestep() = 0;
    virtual void make_galton_board(int numPins) = 0;
    virtual void rasterize_circle(float x, float y, float r, Color color) = 0;
...
public:
  ...
  RasterizerImp(PixelSampleMethod psm, LevelSampleMethod lsm,
    ...
    size_t get_supersample_index(size_t x, size_t y);
    void add_liquid(size_t x, size_t y);
    void timestep();
    void initialize_liquid();
    void rasterize_circle(float x, float y, float r, Color color);
    void make_galton_board(int numPins);
    // Rasterize a point
  // P0 = (x, y)
